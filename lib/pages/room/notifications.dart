...
class _NotificationActivitiesState extends State<NotificationActivities> {
  ...
  @override
  void initState() {
    stream = activitiesRef.where("to", isEqualTo: Get.find<UserController>().user.uid).orderBy("time", descending: true).snapshots().listen((event) {
      activities.clear();
        if(event.docs.length > 0){
          event.docs.forEach((element) {
            activities.add(ActivityItem.fromJson(element.data()));
          });
        }
          setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      child: Column(
        mainAxisAlignment: activities.length == 0 ? MainAxisAlignment.center : MainAxisAlignment.start,
        children: [
          if(activities.length == 0) noDataWidget("No Activities yet", fontsize: 21),
          if(activities.length > 0)Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Container(
                        ...
                        child: Text(Functions.timeAgoSinceDate(DateFormat("dd-MM-yyyy h:mma").format(activities[index].time.toDate())),
